...
export function assertSeedCapability(scenario: Scenario, driver: SeedCapableDriver): void {
  ...
  const supported = driver.supportsSeedStrategy?.(strategy) ?? false;
  if (!supported) {
...
export async function runSeedPhase(
  ...
  if (seed?.strategy === "inject") {
    ...
    return { conversation, turns: [], seedCompletedAt: now() };
  }
  ...
  if (seed.strategy === "fork") {
    ...
    return { conversation: forked, turns: seedTurns, seedCompletedAt: now() };
  }
